import { motion } from 'framer-motion'
import { useAuth } from '../hooks/useAuth'

export default function LoginPage() {
  const { signIn, loading, error } = useAuth()

  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4">
      {/* Top maroon bar */}
      <div className="fixed top-0 left-0 right-0 h-1.5 bg-maroon" />

      <motion.div
        initial={{ opacity: 0, y: 32 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="w-full max-w-md text-center"
      >
        {/* Logo mark */}
        <div className="mx-auto mb-6 w-16 h-16 rounded-2xl bg-maroon flex items-center justify-center shadow-lg">
          <span className="text-white text-2xl font-display font-bold">IP</span>
        </div>

        <h1 className="text-4xl font-display font-bold text-gray-900 mb-2">
          IITM Pulse
        </h1>
        <p className="text-gray-500 mb-10 text-base">
          Track assignments, deadlines & projects — for IITM BS-DS students
        </p>

        {error && (
          <div className="mb-6 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
            {error}
          </div>
        )}

        <motion.button
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.97 }}
          onClick={signIn}
          disabled={loading}
          className="w-full py-4 px-6 bg-maroon text-white rounded-2xl font-display font-semibold text-base
                     hover:bg-maroon-light transition-colors disabled:opacity-60 shadow-lg
                     flex items-center justify-center gap-3"
        >
          {/* Google icon */}
          <svg className="w-5 h-5 fill-white" viewBox="0 0 24 24">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          {loading ? 'Signing in…' : 'Sign in with Google'}
        </motion.button>

        <p className="mt-6 text-xs text-gray-400">
          Only <span className="font-medium">@study.iitm.ac.in</span> and <span className="font-medium">@ds.study.iitm.ac.in</span> emails are accepted
        </p>
      </motion.div>
    </div>
  )
}
